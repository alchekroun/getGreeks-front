            ...
            <v-range-slider v-model="ranges.rangeStock.range"
              ...
              <template v-slot:prepend>
                <v-text-field
                  ...
                  type="number"
                  style="width: 60px"
                  @change="$set(ranges.rangeStock.range, 0, $event); calculateGamma"
                ></v-text-field>
              </template>
  ...
  methods: {
    async calculateGamma() {
      ...
      if (spotB && spotE && strike && drift && rate && expiration) {
        const path = `http://localhost:5000/api/calc/gamma/${spotB}/${spotE}/${strike}/${drift}/${rate}/${expiration}/`;
